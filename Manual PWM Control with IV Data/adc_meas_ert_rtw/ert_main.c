 ...
 * C/C++ source code generated on : Wed Jul  6 12:10:24 2022
 *
...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) adc_meas_M->Timing.clockTick0;
  adc_meas_step();
...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(adc_meas_M, 0);
  ...
  errorCode = extmodeInit(adc_meas_M->extModeInfo, &rtmGetTFinal(adc_meas_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(adc_meas_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(adc_meas_M);
  enableTimer0Interrupt();
  globalInterruptEnable();
  while (runModel) {
