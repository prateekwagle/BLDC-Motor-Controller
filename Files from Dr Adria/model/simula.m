% Aquest script permet simular i mostrar els resultats de la simulacio dels
% models de la maquina sincrona.
% 
% Es poden simular dos models alternatius: 
% 
% * mode amb controls de temps discret i model promitjat de convertidor.
% 
% * modeSPS amb controls de temps discret i model de convertidor amb
%  commutacions (requereix tenir la Toolbox SimPowerSystems).
...
%     subplot(1,1,1);
    if(exist('FILTRA'))
        graficamela({captura.t},{(lsim(FVVLP,captura.Generador.isabc(:,1).*captura.Generador.vsabc(:,1)+captura.Generador.isabc(:,2).*captura.Generador.vsabc(:,2)+captura.Generador.isabc(:,3).*captura.Generador.vsabc(:,3))./lsim(FVVLP,captura.Generador.parell_m.*captura.Generador.omega_m))*100},{'b','g','r'},{'Temps [s]','Rendiment Estàtor [%]'},{'Rendiment Estàtor Isd=0','Rendiment Estàtor Qs=0','Rendimemt Pont de diodes'},[t0,t1 80 100, 0.1]);  

    else
        graficamela({captura.t},{((captura.Generador.isabc(:,1).*captura.Generador.vsabc(:,1)+captura.Generador.isabc(:,2).*captura.Generador.vsabc(:,2)+captura.Generador.isabc(:,3).*captura.Generador.vsabc(:,3))./(captura.Generador.parell_m.*captura.Generador.omega_m))*100},{'b','g','r'},{'Temps [s]','Rendiment Estàtor [%]'},{'Rendiment Estàtor Isd=0','Rendiment Estàtor Qs=0','Rendimemt Pont de diodes'},[t0,t1 -100 100, 0.1]);  
    end
