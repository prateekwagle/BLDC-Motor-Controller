...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) integrating_svpwm_M->Timing.clockTick0;
  integrating_svpwm_step();
...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(integrating_svpwm_M, 0);
  ...
  integrating_svpwm_initialize();
  globalInterruptDisable();
  globalInterruptEnable();

  /* External Mode initialization */
  errorCode = extmodeInit(integrating_svpwm_M->extModeInfo, &rtmGetTFinal
    (integrating_svpwm_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(integrating_svpwm_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(integrating_svpwm_M);
  enableTimer0Interrupt();
  enable_interrupts();
  globalInterruptEnable();
  while (runModel) {
