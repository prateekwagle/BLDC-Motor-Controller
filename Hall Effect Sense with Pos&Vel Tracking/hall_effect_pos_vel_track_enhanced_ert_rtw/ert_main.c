...
void rt_OneStep(void)
{
  ...
  hall_effect_pos_vel_track_enhanced_step();
...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(hall_effect_pos_vel_track_en_M, 0);
  ...
  hall_effect_pos_vel_track_enhanced_initialize();
  globalInterruptDisable();
  globalInterruptEnable();

  /* External Mode initialization */
  errorCode = extmodeInit(hall_effect_pos_vel_track_en_M->extModeInfo,
    &rtmGetTFinal(hall_effect_pos_vel_track_en_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(hall_effect_pos_vel_track_en_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(hall_effect_pos_vel_track_en_M);
  enableTimer0Interrupt();
  enable_interrupts();
  globalInterruptEnable();
  while (runModel) {
  ...
  /* Terminate model */
  hall_effect_pos_vel_track_enhanced_terminate();
